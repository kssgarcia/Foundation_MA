...
from utils.SIMP_utils_test import sparse_assem, strain_els
...
ax[0].imshow(np.flipud(-mats[:,0].reshape(nx,ny)), cmap='gray', interpolation='none', norm=colors.Normalize(vmin=-Emax,vmax=Emin))
ax[0].set_title('Mats density initial')
ax[1].imshow(np.flipud(-sensi_number.reshape(nx,ny)), cmap='gray', interpolation='none', norm=colors.Normalize(vmin=-1,vmax=0))
